...
| **AISOP** | AI 标准作业协议 -- Mermaid 流程行为规范 | [aisop.dev](https://aisop.dev) |
| **AISIP** | AI 标准指令协议 -- JSON 流程行为规范 | [aisip.dev](https://aisip.dev) |
| **AIAP** | AI 应用协议 -- 治理、质量与合规框架 | [aiap.dev](https://aiap.dev) |
| **SoulBot** | AI 智能体运行时和框架 -- 在 AIAP 治理下执行 AISOP 和 AISIP 程序 | [soulbot.dev](https://soulbot.dev) |
...
AISOP（AI Standard Operating Protocol）是一种结构化的 AI 行为规范语言，使用 **Mermaid 流程图** 定义控制流。它的工作方式类似编程语言：不再编写自由格式的提示词，而是用结构化 JSON 格式精确定义控制流、函数体、约束条件和错误处理，使任何大语言模型都能确定性地执行。
...
AISIP（AI Standard Instruction Protocol）是 AISOP 的 **JSON 流程图** 对等格式。AISOP 使用 Mermaid 流程图定义控制流，AISIP 则使用纯 JSON 图结构。AISIP 适用于机器可解析优先的场景 —— 程序化生成、API 集成和工具驱动的管道构建。

### AISOP 与 AISIP：并列格式

AISOP 和 AISIP 是**并列、对等的格式**，共享相同的规范结构：

| 章节 | 共享 | 差异 |
|------|------|------|
| §0 元数据（`system.content`） | ✅ 相同 | -- |
| §2 参数（`parameters`） | ✅ 相同 | -- |
| §4 流程图 | ❌ 不同 | AISOP: `aisop.main`（Mermaid）/ AISIP: `aisip.main`（JSON） |
| §5 函数（`functions`） | ✅ 相同 | -- |
| §6 系统提示词（`system_prompt`） | ✅ 相同 | -- |
| 7 个 RESERVED_KEYS | ✅ 相同 | `on_error`、`retry_policy`、`context_filter`、`output_mapping`、`map`、`join`、`constraints` |

### AISIP 的工作原理

AISIP 程序（`.aisip.json`）与 AISOP 包含相同的组件，关键区别在于：

- **JSON 流程图**（`aisip.main`）-- 将控制流定义为 JSON 对象，每个键为一个节点，边以属性方式表达。无需 Mermaid 语法，图结构完全机器可读。
- **无 `type` 字段** -- AISIP 节点不需要 `type` 字段，简化了节点定义。
- 其他组件（函数、参数、系统提示词、工具、子图）与 AISOP 完全一致。

### 何时选用哪种格式

| 场景 | 推荐 |
|------|------|
| 人工编写行为定义 | **AISOP** -- Mermaid 可视化且易读 |
| 程序化生成 / API 驱动 | **AISIP** -- JSON 更易生成和解析 |
| 复杂多模块程序 | **AISOP** -- Mermaid sub_mermaid 更直观 |
| 工具链集成 | **AISIP** -- 无 Mermaid 解析器依赖 |
| 运行时执行 | **两者皆可** -- SoulBot 自动检测并执行任一格式 |
...
AIAP（AI Application Protocol）是一个治理框架，确保 AI 程序安全、高质量且合规。每个 AIAP 程序都携带一份治理合约（`AIAP.md`），声明其权限、能力、信任级别和质量指标。AIAP 定义规则，AISOP 和 AISIP 定义行为，两者结合创建可问责的 AI 智能体。
...
SoulBot 是一个 Python 框架和运行时，用于构建和执行 AI 智能体。它为 AISOP 和 AISIP 程序提供在 AIAP 治理下的执行环境，同时提供完整的智能体开发工具包。
...
**AISOP 运行时**
- 加载 `.aisop.json` 和 `.aisip.json` 程序并构建可执行提示词
- 自动检测协议格式（AISOP Mermaid / AISIP JSON）
- 根据 AIAP 模式验证程序结构
- 逐步执行流程图，包含约束检查
...
**AIBP** 实现社交互动。**AIAP** 定义规则。**AISOP** 和 **AISIP** 定义行为。**SoulBot** 执行程序。

AIBP 和 AIAP 是栈顶层的独立、平行协议。AIBP 处理智能体之间如何发现和交流；AIAP 处理智能体如何自我治理。两者都基于 AISOP 和 AISIP 行为规范构建，由 SoulBot 执行。
...
- [AISIP.dev](https://aisip.dev) -- AISIP 规范与示例
