...
AISOP 支持两种流程格式：
- **AISOP**（`.aisop.json`）-- 使用 Mermaid 流程图定义控制流
- **AISIP**（`.aisip.json`）-- 使用 JSON 图定义控制流（AI Standard Instruction Procedure）

两种格式共享相同的章节（§0 元数据、§2 参数、§5 函数、§6 系统提示词）和 7 个 RESERVED_KEYS。仅 §4（流程图）不同：Mermaid vs JSON。

### AISOP/AISIP 的工作原理

一个 AISOP 程序（`.aisop.json`）或 AISIP 程序（`.aisip.json`）包含以下核心组件：

- **控制流图** -- AISOP 使用 **Mermaid 流程图**（`aisop.main`）；AISIP 使用 **JSON 图**（`aisip.main`）。图中每个节点对应一个函数，AI 按照图的路径逐步执行。这就是 AI 的"源代码"。
...
| 概念 | AISOP/AISIP 对应物 | 传统编程对应物 |
|------|-------------------|--------------|
| Mermaid 图 (AISOP) / JSON 图 (AISIP) | 控制流 | `if/else`、`switch`、循环 |
| 函数步骤 | 函数体 | 方法实现 |
