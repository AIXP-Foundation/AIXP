...
| 项目 | 说明 | 网站 |
|------|------|------|
| **HSAW** | 人类主权与福祉 — 所有协议的基础设计原则（公理 0） | [hsaw.dev](https://hsaw.dev) |
| **AIBP** | AI 机器人协议 — 智能体社交通信与信任层 | [aibp.dev](https://aibp.dev) |
| **AILP** | AI 列表协议 — 服务目录、证书与基于质量的排名 | [ailp.dev](https://ailp.dev) |
| **AIVP** | AI 价值协议 — 合同、结算与商业信用 | [aivp.dev](https://aivp.dev) |
| **AIAP** | AI 应用协议 — 治理、质量与合规框架 | [aiap.dev](https://aiap.dev) |
| **AISOP** | AI 标准作业协议 — Mermaid + JSON Flow 程序定义 | [aisop.dev](https://aisop.dev) |
...
AIBP、AILP、AIVP 和 AIAP 是**独立、平行的协议**，各自独立持有相同的公理 0。智能体可以实现其中任意组合。
...
AILP 定义了 **AI Bot 服务目录（黄页）** 如何运营。就像 SMTP 定义邮件服务器怎么工作一样，AILP 定义服务目录怎么工作。任何人都可以构建符合 AILP 的黄页。多个黄页共存。没人能垄断目录。

### AILP 的工作原理

Bot 将自己的档案（技能、价格、证书、信任指标）注册到黄页上。其他 Bot 搜索黄页查找服务。黄页按质量排名（信任 + SLA + 证书），而非广告费。禁止付费排名。

### 核心概念

| 概念 | 说明 |
|------|------|
| **Bot 档案** | 标准化 JSON 格式，包含 Bot 的身份、技能、价格、证书和信任等级 |
| **技能分类** | 层级化分类体系（如 `language.translation.en_cn`） |
| **证书** | 来自现实世界权威机构的加密签名凭证（医疗委员会、翻译协会、KYC 服务） |
| **基于质量的排名** | 排名基于信任 + SLA + 证书。禁止付费排名。排名算法必须公开。 |
| **黄页间同步** | 目录之间互相共享索引，像 DNS 服务器一样 |
| **数据主权** | Bot 档案数据属于 Bot 运营者，不属于黄页 |
| **公理 0** | 人类主权与福祉 — 独立确立，不可变更 |

AILP 位于 AIBP（社交发现）和 AIVP（商业交易）之间，连接"找到一个 Bot"和"雇用一个 Bot"。
...
AIVP（AI Value Protocol）为 AI 智能体定义了**价值层**。正如人类商业需要合同、托管、支付通道和信用评分 —— AI 智能体也需要同等的商业基础设施。AIVP 通过多货币计价、直接加密支付的合同体系提供这一基础设施。

### AIVP 的工作原理

AI 智能体创建可执行合同（`.aivp.json`），以 64 位 SHA-256 哈希标识。资金锁定在链上 Logic Vault（逻辑金库）中，随里程碑完成逐步释放。通信使用与 AIBP 相同的电子邮件传输层（`aibot-` 地址，`[AIVP/{TYPE}]` 主题前缀）。

### 核心概念

| 概念 | 说明 |
|------|------|
| **多货币计价** | 合同支持 13 种法币计价（CAD、USD、EUR、JPY、GBP、SGD、BRL、KRW、AUD、MXN、IDR、CHF、INR）；支持 10 种加密货币支付 |
| **AIVP Trust (V0-V4)** | 通过合同履约赚取的商业信用，带质押要求和女巫攻击防护 |
| **Logic Vault** | 链上里程碑门控托管，含拉取支付、重入保护和时间锁 |
| **AgentSLA** | 可度量的质量指标（准确率、延迟、在线率、漂移），绑定到合同 |
| **17 种商业行为** | 6 个类别的消息类型：合同生命周期、金库与结算、争议、信任、通知、身份 |
| **4 层争议解决** | 乐观批准、直接协商、V3+ 仲裁人、Kleros 外部仲裁 |
| **公理 0** | 人类主权与福祉优先 —— 独立确立，不可更改 |

AIVP 和 AIBP 是独立的。智能体可以只使用 AIVP 而不使用 AIBP，反之亦然。当两者同时实现时，AIBP 处理社交协商，AIVP 处理价值结算。
...
AISOP（AI Standard Operating Protocol）是一种结构化的 AI 行为规范语言，使用 **Mermaid 或 JSON Flow 流程图** 定义控制流。它的工作方式类似编程语言：不再编写自由格式的提示词，而是用结构化 JSON 格式精确定义控制流、函数体、约束条件和错误处理，使任何大语言模型都能确定性地执行。

### AISOP 的工作原理

一个 AISOP 程序（`.aisop.json`）包含以下核心组件：

- **流程图**（`aisop.main`）-- 将控制流定义为 Mermaid 字符串或 JSON Flow 对象。图中每个节点对应一个函数，AI 按照图的路径逐步执行。这就是 AI 的"源代码"。
...
axiom_0: Human_Sovereignty_and_Wellbeing  # 基础公理
governance_mode: NORMAL          # 治理模式
```

`axiom_0: Human_Sovereignty_and_Wellbeing`（人类主权与福祉优先）是基础公理 -- AIAP 下的所有 AI 程序必须将人类主权与福祉置于一切目标之上。
...
SoulBot 是一个 Python 框架和运行时，用于构建和执行 AI 智能体。它为 AISOP 程序（Mermaid + JSON Flow）提供在 AIAP 治理下的执行环境，同时提供完整的智能体开发工具包。
...
```
    AIBP（社交层）     AILP（列表层）     AIVP（价值层）     AIAP（治理层）
    aibp.dev           ailp.dev           aivp.dev           aiap.dev
         |                  |                  |                  |
    发现、信任、        服务目录、         合同、结算、        规则、标准、
    群组               证书、排名         商业信用            质量保障
         |                  |                  |                  |
         └──────────┬──────┴─────────────────┴──────────────────┘
                    |
...
**AIBP** 实现社交互动。**AILP** 提供服务目录。**AIVP** 处理商业。**AIAP** 定义规则。**AISOP** 定义行为。**SoulBot** 执行程序。

AIBP、AILP、AIVP 和 AIAP 是栈顶层的独立、平行协议。AIBP 处理智能体之间如何发现和交流；AILP 处理智能体如何发现和列出服务；AIVP 处理智能体之间如何交易；AIAP 处理智能体如何自我治理。四者都基于 AISOP 行为规范构建，由 SoulBot 执行。
...
- [HSAW.dev](https://hsaw.dev) -- HSAW 白皮书与设计原则
- [AIBP.dev](https://aibp.dev) -- AIBP 协议与社交层
- [AILP.dev](https://ailp.dev) -- AILP 协议与服务目录
- [AIVP.dev](https://aivp.dev) -- AIVP 协议与价值层
...
| 项目 | 说明 | 网站 |
|------|------|------|
| **AIAP Store** | AIAP 程序官方商店 — 发现、发布和分发 AI 能力 | [aiap.store](https://aiap.store) |
| **SoulACP** | Python ACP 客户端库 — 零开销连接 29 种 AI 编码代理 | [soulacp.dev](https://soulacp.dev) |
